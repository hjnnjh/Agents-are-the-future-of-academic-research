...
在 [第一篇文章](/posts/agent-basics/2026-02-13-llm-agent-basics) 中，我们了解了单个 Agent 的核心能力：感知、推理、行动。但单个 Agent 存在三个明显的局限：
...
了解了协作模式后，我们来看看当前主流的多 Agent 框架。选择框架时主要考虑三点：上手难度、支持的协作模式、社区活跃度。
...
微软研究院推出的 AutoGen 在学术界使用广泛。它的核心思想是 Agent 之间通过对话来协作，类似一场有组织的学术研讨会。

AutoGen 的 `GroupChat` 机制允许多个 Agent 在一个共享的聊天室中讨论问题，由一个 `GroupChatManager` 来决定下一个发言的 Agent。这种设计适合学术场景中的讨论式任务，比如让一个"方法论专家 Agent"和"统计学专家 Agent"就实验设计方案展开对话。
...
OpenAI 在 2025 年推出的 Agents SDK 提供了一套轻量级的原语。其中两个核心概念：
...
在主流框架之外，Anthropic 提出了一套更偏向工程实践的多 Agent 设计模式。

### Orchestrator-Workers：学术科研常用模式
...
另一种常用模式是 **Evaluator-Optimizer**：一个 Agent 负责生成（Generator），另一个 Agent 负责评估（Evaluator），两者交替执行直到输出满足质量标准。
...
这种自我迭代的闭环很适合学术场景，因为它模拟的就是现实中"写作→审稿→修订"的流程。
...
这个案例说明多 Agent 协作在学术场景中的用法：**把重复性的科研工作流程自动化，而不是替代研究者。**
...
多 Agent 协作也有不少坑。NeurIPS 2025 上发表的 MAST 分类法（Cemri et al.）分析了 1600+ 条多 Agent 交互记录，识别出 **14 种失败模式**，分为四大类：任务规划失败、Agent 间通信失败、工具使用失败和推理执行失败。
...
当最终结果出错时，定位问题来源比较困难，你需要逐一回溯每个 Agent 的输入输出。
...
1. **单 Agent 有天花板，多 Agent 是自然的延伸**。当任务涉及多领域知识、需要多角度审视、或单个上下文窗口无法承载时，多 Agent 协作是可行的方案。但多 Agent 不是炫技工具，只有真正需要时才值得引入。
2. **协作模式比框架选型更重要**。顺序、并行、层级、辩论四种模式可以灵活组合。在学术场景中，"并行检索 + 顺序整合 + 辩论验证"是一种验证过的组合。
3. **上下文隔离决定多 Agent 的效果**。每个 Agent 只应看到它需要的信息。这与 [上下文工程](/posts/agent-basics/2026-02-15-context-engineering) 中讨论的"隔离"策略一脉相承，在多 Agent 系统中尤其如此。
