...
LLM 再聪明，它只能"想"和"说"——你问它怎么分析数据，它能给你一份方案，但它没法真的去执行。Agent 则可以在思考之后 **动手做**：搜索网页、读写文件、运行代码、调用 API。大脑加上手脚，才是一个能干活的人。
...
在 Function Call 出现之前，大模型只能生成文本。有了它，模型在推理过程中发现信息不够或需要执行操作时，可以主动发出请求："我需要调用这个工具，参数如下，请帮我执行并返回结果。"
...
Prompt Engineering 解决的是"怎么让模型说得更好"，Function Call 解决的是"让模型能动手做事"。它把模型的能力边界从文本生成扩展到了实际操作：

- 通过搜索工具获取实时信息
- 通过数据库接口读取内部数据
- 发送邮件、操作日程，甚至控制实验室设备

当模型能在合适的时机调用合适的工具，它就从聊天窗口变成了能闭环解决问题的 Agent。
...
回到学术研究场景。Agent 的能力适合研究中常见的多步骤、多工具任务：
...
**Coding Agent**（如 Claude Code、OpenCode、Cursor 等）是目前落地最好的 Agent 类型之一。很多人（包括作者本人）认为 Coding Agent 本质上就是通用 Agent，这也是本博客重点关注的方向。
...
2. **Function Call 是连接模型与外部世界的桥梁**——让模型从"只能说"变成"能动手"
