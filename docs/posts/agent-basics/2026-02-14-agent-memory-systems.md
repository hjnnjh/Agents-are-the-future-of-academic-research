...
这不是 AI 的"健忘"——这是 **记忆系统设计** 的问题。理解 Agent 如何管理记忆，才能让 AI 在长期科研任务中持续发挥作用。

## 为什么 Agent 需要记忆？

在 [上一篇文章](/posts/agent-basics/2026-02-13-llm-agent-basics) 中，我们讲了 Agent 的核心循环：感知 → 推理 → 行动。但有一个问题被简化了：**Agent 在推理时，依据的是什么信息？**
...
看起来不小？但实际研究中，当你处理几十篇论文、数千行代码、多轮对话历史时，上下文窗口很快就会被填满。而且**窗口越大，模型的"注意力"就越分散**——塞进去 500 页文档，模型反而可能忽略其中的关键信息。
...
工作记忆就是当前上下文窗口中的内容。容量有限但读写快，是 Agent 推理决策的直接依据。
...
2023 年，UC Berkeley 的研究团队提出了 **MemGPT**（现已更名为 Letta），是 Agent 记忆管理的代表性方案。

MemGPT 的思路是：**把 LLM 的上下文窗口类比为操作系统的内存，用虚拟内存的思想来管理它**。
...
这些操作由 Agent 自己发起，而不是由程序硬编码的规则控制。Agent 自己来决定记忆的管理。

::: tip 对比传统方法
传统方法（如滑动窗口）是"被动遗忘"——信息按时间顺序被丢弃，不管重不重要。MemGPT 则让 Agent 自己判断哪些信息该保留、哪些该存档，是一种主动管理的方式。
:::
...
**RAG (Retrieval-Augmented Generation，检索增强生成)** 是目前用得最多的长期记忆实现方案。思路很直接：
...
Claude Code 是目前比较成熟的 Coding Agent，它的记忆管理设计可以作为参考：
...
了解了 Agent 的记忆机制后，来看看它在学术场景中的具体用法：
...
2. **核心挑战是"有限窗口 vs 无限信息"**——好的记忆系统不是记住所有东西，而是在需要的时候提供对的信息
3. **RAG 是目前最实用的长期记忆方案**——通过向量检索让 Agent 按需"回忆"；MemGPT 的虚拟记忆思路则尝试让 Agent 自主管理记忆
