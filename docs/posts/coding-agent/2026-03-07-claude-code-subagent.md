...
summary: "Subagent 是 Claude Code 内置的并行任务机制，让多个子代理同时处理独立子任务，各自拥有独立的上下文窗口。本文介绍 Subagent 的工作原理、内置与自定义 Subagent 的使用方法，以及配合 CCR 实现差异化模型路由的实践。"
featured: false
---
...
用 Claude Code 处理大型任务时，你可能遇到过这样的情况：要重构一个跨越几十个文件的模块，要同时分析多条数据处理管道，或者要在编写代码的同时审查安全风险。这些任务如果让一个 Agent 串行完成，既慢，上下文也会越积越长，前面分析的细节容易被"遗忘"。
...
| `description` | 是 | 决定 Claude 何时自动调用此代理，写清楚适用场景 |
...
**大规模代码重构**：需要在 50 个文件中替换一个废弃函数时，让主 Agent 用 Explore 找出所有调用位置，然后为每个文件启动一个独立 Subagent 执行替换。每个 Subagent 的上下文只包含单个文件，不容易出错。

**多维度代码审查**：同时启动"代码风格检查"、"安全审查"和"测试覆盖率分析"三个 Subagent 并行审查，省去串行等待的时间。
...
当你使用 [CCR（Claude Code Router）](/posts/coding-agent/2026-03-07-claude-code-router) 时，可以将不同模型层级的请求路由到不同服务商：轻量 Subagent 走廉价本地模型，推理密集的 Subagent 走高性能 API。
...
**并行数据处理**：实验产出了 10 批数据，每批需要独立清洗和统计分析。用 Subagent 并行处理，不用逐批串行等待。

**多维度文献分析**：同时对一篇长论文的方法论、实验设计和结论部分分别启动 Subagent 并行阅读，最后由主 Agent 生成综合摘要。

**成本控制**：通过 CCR 差异化路由，把文件扫描、格式检查等辅助任务交给本地或廉价模型，把真正需要深度思考的实验规划任务交给高性能模型。对 API 额度有限的学生或独立研究者来说，这能省不少钱。

---

## 小结

Subagent 系统让 Claude Code 可以同时运行多个独立的子任务：
...
| **并行执行** | 独立子任务同时处理，减少等待时间 |
| **CCR 差异化路由** | 不同复杂度的 Subagent 走不同的模型和服务商，按需分配成本 |

可以从一个简单的自定义 Subagent 开始：找一个你在项目中频繁重复的操作，把它封装成 Subagent 试试看。
