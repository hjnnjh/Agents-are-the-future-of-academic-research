...
**Skills** 解决的就是这个问题。Skill 是一个打包好的指令集——你把某类任务的完整操作流程、输出规范、工具约束都写在一个文件里，之后只需一句 `/skill-name` 就能激活，Claude 会按你设定的流程工作，无需重复解释。

官方的定义是：**"教会 Claude 以可重复的方式完成特定任务"**。
...
简单的判断标准：如果某条规则几乎每次任务都用到，放 CLAUDE.md；如果只在特定工作流中需要，做成 Skill，按需激活。
...
### `description` 字段：决定自动激活的准确率

每次对话中，Claude 会用约 100 tokens 扫描所有 Skill 的 `description` 字段，判断是否与当前任务相关——相关则自动加载完整 Skill 内容。

`description` 的写法直接影响自动激活的准确率。建议用"当……时"或"适用于……场景"的句式，从 Claude 的决策视角描述触发条件，而非只说功能：
...
这个 Skill 的设计要点：通过明确写出"禁止使用写入工具"，在指令层面实现了只读约束，将审查和修改职责分离，避免审查 Agent 直接改动文件而绕过人工确认。
...
包含脚本的 Skill 可以做到更多。SKILL.md 中的代码片段是**参考实现**——Claude 阅读这些代码后理解处理逻辑，然后通过 Bash 工具执行真实命令，或者调用 `scripts/` 目录下预置的脚本文件。
...
单个 Skill 处理单类任务，多个 Skill 可以组合成工作流。本项目 README 中记录了几条实战工作流：
...
调用方式：依次手动触发每个 Skill，或在 prompt 中告诉 Claude 按顺序执行这些步骤。
...
手动克隆 + 复制 Skill 的方式适合偶尔安装，但社区已经有一些 npm CLI 工具可以简化 Skill 的安装、更新和管理。
...
工作原理：openskills 会在 `AGENTS.md` 中生成标准化的 `<available_skills>` XML 块，非 Claude Code 工具（如 Cursor）也能通过这个文件识别和使用 Skill。
...
[vercel-labs/skills](https://github.com/vercel-labs/skills) 是另一个 Skill 管理 CLI，对应的技能发现网站是 [skills.sh](https://skills.sh)。与 openskills 侧重兼容性不同，`skills` 更偏向 Skill 的发现与分发——支持从任意 GitHub 仓库安装技能，并用符号链接（symlink）保持单一来源，方便更新。
...
Vercel 团队维护的 `agent-skills` 技能集（`vercel-labs/agent-skills`）包含：
...
审查类 Skill 有个共同特点：`content-reviewer` 和 `markdown-reviewer` 都声明禁止使用 Edit/Write 工具，只能读取，实现"审查-修改"职责分离，审查结果需人工确认后才决定是否修改。
...
建议从你最常重复的任务入手——哪怕只是把"每次都要重复说明的数据分析规范"写成一个 Skill，就能省掉不少重复沟通。

::: tip 与 Hooks 配合
本博客项目的 [skill-eval-local.sh Hook](/posts/coding-agent/2026-03-07-claude-code-hooks) 在每次任务提交时自动列出所有 Skill 并强制 Claude 评估，避免遗漏。如果你的项目积累了多个 Skill，结合类似的 Hook 可以提高自动激活的准确率。
:::
